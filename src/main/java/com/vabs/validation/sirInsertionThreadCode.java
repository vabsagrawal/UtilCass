 ...
 * 
 */
public class sirInsertionThreadCode extends Thread {
	private List list1 = null;

	public sirInsertionThreadCode(List list1) {
		this.list1 = list1;
	}
	...
	public void run() {
		...
		try {
			...
			for (int j = 0; j < list1.size(); j++) {
				...
				System.out.println("Item  Count in PROD SIR is " + item + "- "
						+ sirDataList.size());
				bound_stmt_delete = delete_sir.bind(Integer.parseInt(item));
				session_qa.execute(bound_stmt_delete);
				System.out.println("Item -" + item + " is deleted from QA SIR");
				int i = 1;
				...
				System.out.println("Item Insertion in QA SIR is completed for item "
						+ item);
